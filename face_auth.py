...
# Verification logic only (no camera)
def verify_face_headless(test_img, user_image):
    if not os.path.exists(user_image):
        print(f"No face registered for this user: {user_image}")
        return False
...
# Authenticate face of expected user (Interactive)
def authenticate_face(expected_user):
    user_image = os.path.join(FACE_DB, expected_user + ".jpg")
    test_img = "test_face.jpg"
    if not capture_face(test_img):
        return False
    return verify_face_headless(test_img, user_image)